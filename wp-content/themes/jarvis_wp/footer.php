...
<?php if($smof_data['rnr_home_type']=="Video") { ?>
        ...
        <a id="rnr-background-video" class="rnr-video-player" data-property="{ videoURL : '<?php echo $smof_data['rnr_home_video_id']; ?>' , containment : '#home-background-video' , mute : <?php echo $smof_data['rnr_video_mute']; ?>, startAt : 0, opacity : 1, optimizeDisplay: true, autoPlay : true, vol: 100, showControls: false, loop: <?php echo $smof_data['rnr_enable_video_loop']; ?>}"></a>	            
 <?php } ?>
...
/*var back = ["#F36F2B","#B2D236","#953F97","#1C97D4","#F05827","#7D1A4E","#EC3E94","#B61F2A","#48C4DB","#46205F","#F5B818"]; 
  var rand = back[Math.floor(Math.random() * back.length)];

jQuery('.header-text').css('background',rand);

jQuery('.title h1, .service-box .service-icon').css('box-shadow','0px 0px 0px 3px '+rand);
jQuery('.highlight').css('color',rand);
jQuery('.img-container').css('background',rand);

jQuery('.img-container:after').css('border','15px solid #000');  
jQuery('.service-icon').css('background-color',rand); 
