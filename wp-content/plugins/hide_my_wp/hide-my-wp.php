...
return $output;
    ...
    function spam_blocker($post_id){
        ...
        $spam= false;
        if ($this->is_permalink() && $this->opt('replace_comments_post') && (!isset($_GET['p_'.$login_query]) || $_GET['p_'.$login_query]!=$this->opt('admin_key'))) {
            $counter['1']++;
            $spam = true;
        }
    ...
    function global_css_filter(){
        ...
        if ( (isset($wp_query->query_vars['style_wrapper']) && $wp_query->query_vars['style_wrapper'] && $this->is_permalink() ) ){
            ...
            if ($this->opt('full_hide')&& $this->opt('admin_key')) {
                if (!isset($_GET['p_' . $login_query]) || $_GET['p_' . $login_query] != $this->opt('admin_key'))
                    return false;
